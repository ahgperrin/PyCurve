...
class NelsonSiegelAugmented:
    ...
    def _second_hump(self, t) -> Union[np.ndarray, float]:
        return self.beta3 * np.subtract(np.array(((1 - np.exp(-np.array(t, np.longdouble) / self.tau2)) /
                                                  (np.array(t, np.longdouble) / self.tau2))),
                                        np.array(np.exp(-np.array(t, np.longdouble) / self.tau2)))
    ...
    def plot_model_params(self):
        ...
        fig = plt.figure(figsize=(12.5, 8))
        fig.suptitle("Nelson Siegel Svensson Parameters")
        fig.canvas.set_window_title('Nelson Siegel Svensson Parameters')
    ...
    def plot_model(self):
        ...
        b0 = np.ones(1000) * self.beta0
        fig = plt.figure()
        fig.suptitle("Nelson Siegel Svensson Components")
        fig.canvas.set_window_title('Nelson Siegel Svensson Components')
