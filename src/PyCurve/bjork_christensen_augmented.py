...
class BjorkChristensenAugmented:
    ...
    def _time_decay(self, t) -> Union[np.ndarray, float]:
        return self.beta1 * (np.array(t) / (2 * self.tau))

    def _hump(self, t) -> Union[np.ndarray, float]:
        return self.beta2 * (np.array(((1 - np.exp(-np.array(t) / self.tau)) /
                                       (np.array(t) / self.tau))))

    def _second_hump(self, t) -> Union[np.ndarray, float]:
        return self.beta3 * np.subtract(np.array(((1 - np.exp(-np.array(t) / self.tau)) /
                                                  (np.array(t) / self.tau))),
                                        np.array(np.exp(-np.array(t) / self.tau)))

    def _third_hump(self, t) -> Union[np.ndarray, float]:
        return self.beta4 * (np.array(((1 - np.exp(-2 * np.array(t) / self.tau)) /
                                       (2 * np.array(t) / self.tau))))
